...
const App: React.FC = () => {
  ...
  const getReadingAndUseOfEnglishRawScore = (): number => {
    ...
    // Parts 1, 2, 3, 7 are 1 mark. Parts 5, 6 are 2 marks. Part 4 is up to 2 marks.
  ...
  const calculateScaleScore = (rawScore: number, points: { low: number, pass: number, high: number }): number => {
      if (rawScore <= points.low) {
        // Extrapolate down from 140
        return Math.round(140 - ((points.low - rawScore) / points.low) * 18); // floor at 122
      }
      ...
      // Extrapolate up from 180, ceiling at 190
      return Math.round(180 + ((rawScore - points.high) / (TOTAL_RAW_SCORES.readingAndUseOfEnglish - points.high)) * 10);
  };
  ...
  const handleCalculate = () => {
    ...
    const readingScale = calculateScaleScore(readingRaw, CONVERSION_POINTS.readingAndUseOfEnglish);
    const writingScale = calculateScaleScore(writingRaw, CONVERSION_POINTS.writing);
    const listeningScale = calculateScaleScore(listeningRaw, CONVERSION_POINTS.listening);
    const speakingScale = calculateScaleScore(speakingRaw, CONVERSION_POINTS.speaking);
